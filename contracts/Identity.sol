...
//     event ContractCreatedEvent(address indexed ownerKey, uint256 creationBlock);

//     modifier isOwner() {
//         require(msg.sender == OwnerKey, "caller is not the Contract Owner");

//         _;
//     }

//     constructor(string memory encoding, bytes memory metadata) public {
//         // TODO: put checks on input into a modifier

//         require(bytes(encoding).length > 0, "nonempty encoding required");
...
//         require(signingKey != address(0), "invalid key set for signer");

//         string memory attrID;
//         Badge storage b = Badges[badgeID];

//         address storedKey = b.signingKey;
//         uint256 length = b.attributeList.length;
...
//         string memory badgeID,
//         string[4][] memory checkedData, // Attestation[] memory checkedData,
...
//         uint256 length = checkedData.length;
...
//             require(a.shared, "attribute does not exist");
...
//     function rejectTrustRelationship(string memory badgeID)
//         public
//         isTrustedSigner(badgeID)
//     {
//         Badge storage b = Badges[badgeID];
//         address signingKey = b.signingKey;
...
//     function appendAttribute(string memory badgeID, string memory attrID)
//         public
//         isOwner()
//     {
//         Badge storage b = Badges[badgeID];
...
//         require(storedKey != address(0), "cannot add attribute to empty badge");

//         Attribute storage a = b.attributes[attrID];

//         require(!a.shared, "cannot add attribute that already exists");
...
//     function resetAttribute(string calldata attrID) external isOwner() {
//         address signingKey;
//         string memory badgeID;

//         string memory aID = attrID;
//         uint256 length = badgesList.length;
...
//     function fullAttribute(string calldata badgeID, string calldata attrID)
//         external
//         view
...
//     function attributeBadgeList(string calldata attrID)
//         external
//         view
//         returns (string[] memory tmpBadgesList)
//     {
//         uint256 ct;
//         uint256 length;
...
//     function fullBadge(string calldata badgeID)
//         external
//         view
//         returns (BadgeView memory)
//     {
//         Badge storage b = Badges[badgeID];
